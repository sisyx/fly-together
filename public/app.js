(function () {
  ...
  function setRole(host) {
    ...
    if (hostRequestWrap)
      hostRequestWrap.style.display = isHost ? "none" : "block";
  ...
  async function refreshSavedList() {
    ...
    const tracksLowerCase = tracks.map((track) => ({
      ...track,
      originalName: track.originalName.toLowerCase(),
    }));
    const searchPhrase = searchInput.value.toLowerCase();
    const searchTrackList = tracksLowerCase.filter((track) =>
      track.originalName.includes(searchPhrase),
    );

    renderSavedList(searchTrackList);
  ...
  function initSocket() {
    ...
    socket.on("connect", () => {
      ...
      searchInput.addEventListener("input", refreshSavedList);
    });
